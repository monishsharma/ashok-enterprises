...
export const calculateTotalTons = (invoices) => {
  return invoices.reduce((totalQty, inv) => {
    if (
      inv.goodsDescription?.type === "KGS" &&
      Array.isArray(inv.goodsDescription.items)
    ) {
...
export const calculateTonsGrowth = ({
...
export const monthlySalesQuery = ({ company, year, month }) => {
  const selctedYear = parseInt(year);
  const selectedMonth = parseInt(month);

  const fyStart = selectedMonth >= 4 ? selctedYear : selctedYear - 1;
  const fyEnd = fyStart + 1;

  const startDate = new Date(fyStart, 3, 1); // April 1st
  const endDate = new Date(fyEnd, 3, 1); // April 1st of next year
...
export const getYearlyTons = async ({
  invoiceCollection,
  company,
  needObj = false,
  year,
...
export const getFYItemBreakdown = async ({
  invoiceCollection,
  company,
  year,
}) => {
