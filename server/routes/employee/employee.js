...
const collectionName =
  process.env.NODE_ENV === "dev" ? "employeeDetails" : "employeeDetails";
...
router.patch("/:employeeId", async (req, res) => {
  try {
    ...
    const result = await db
      ...
      .updateOne({ _id: new ObjectId(employeeId) }, [
        {
          $set: {
            attendance: {
              $cond: {
  ...
  await db
    ...
    .findOneAndUpdate(
      ...
      },
      {
        $set: {
          name: payload.name,
          salaryPerDay: payload.salaryPerDay,
          esi: payload.esi,
...
router.patch("/checkout/all", async (req, res) => {
  ...
  if (
    ...
    !updatedFields.totalWorkingHours
  ) {
...
router.get("/generate-salary-slip-pdf", async (req, res) => {
  ...
  try {
    ...
    const employeesData = results
      .map((emp) => {
        const totalWorkingMinutes = emp.attendance.reduce(
          (sum, day) => sum + (day.totalWorkingHours?.min || 0),
          0,
        );
        const totalOTMinutes = emp.attendance.reduce(
          ...
          days: emp.attendance.map((d) => {
            ...
            return {
              ...
              formattedCheckinTime: d.checkinTime
                ? moment
                    .tz(Number(d.checkinTime), "Asia/Kolkata")
                    .format("HH:mm")
                : "--:--",
              formattedCheckoutTime: d.checkoutTime
                ? moment
                    .tz(Number(d.checkoutTime), "Asia/Kolkata")
                    .format("HH:mm")
                : "--:--",
    ...
    const html = await ejs.renderFile(
      path.join(
        __dirname,
        "../billing/invoice-config/templates/salarySlip.ejs",
      ),
      {
        employeesData,
      },
