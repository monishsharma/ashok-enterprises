...
const Attendance = ({
  ...
  const handleCheckoutAttendance = async ({ rowData, punchedTime }) => {
    ...
    if (eDetail.checkinTime) {
      const storedPunchedInTime = new Date(
        parseInt(eAttendance.checkinTime),
      ).getHours();
    ...
    try {
      ...
      ====================================================== */
        ...
        const shiftStart = moment(
          `${d.DateString} ${isSunday ? SUNDAY_SHIFT_START : SHIFT_START}`,
          "DD/MM/YYYY HH:mm",
        );

        const graceEnd = moment(
          `${d.DateString} ${isSunday ? SUNDAY_CHECKIN_GRACE : CHECKIN_GRACE}`,
          "DD/MM/YYYY HH:mm",
        );
        ...
        if (isSunday) {
          // 🔴 SUNDAY: FORCE MAX 14:00
          finalCheckout = moment.min(actualCheckout, shiftEnd);
        } else {
        ...
        if (!isSunday) {
          otMinutes = finalCheckout.diff(shiftEnd, "minutes");
...
const mapStateToProps = ({ employee: { data: employeeData } }) => ({
  employeeData,
});
